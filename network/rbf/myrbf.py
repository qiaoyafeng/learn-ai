...
class RbfNet:
    ...
    def fit(self, X, y, units, epochs, optimizer):
        ...
        for _ in range(epochs):
            ...
            for i in range(self.__units):
                g_centers[i] = self.__weights[i] * np.mean(
                    (residual * outs[:, i]).reshape((-1, 1)) * (X - self.__centers[i]), axis=0) / (
                                           self.__sigmas[i] ** 2)
                g_sigmas[i] = self.__weights[i] * np.mean(
                    residual * outs[:, i] * (np.linalg.norm(X - self.__centers[i], axis=1) ** 2), axis=0) / (
                                          self.__sigmas[i] ** 3)
